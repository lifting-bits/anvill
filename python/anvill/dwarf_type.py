...
POINTER_TYPES = {"DW_TAG_pointer_type": "*"}

ARRAY_TYPES = {"DW_TAG_array_type"}
  ...
  """
        ...
        if has_type(die):
            ...
            try:
                ...
                raise ParseException(
                    "_process_indirect_types: die offset {:x} is not in typemap".format(
                        die.offset
                    )
                )
    ...
    def _process_pointer_types(cls, die):
        if has_type(die):
            ...
            name = (
                cls._dw_types_cache[offset].name
                if offset in cls._dw_types_cache
                else "UNKNOWN"
            ) + "*"
            base_offset = (
                cls._dw_types_cache[offset].offset
                if offset in cls._dw_types_cache
                else -1
            )
    ...
    def _process_array_types(cls, die):
        if has_type(die):
            ...
            try:
                ...
                raise ParseException(
                    "_process_array_types: die offset {:x} is not in typemap".format(
                        die.offset
                    )
                )
    ...
    def _get_type_die(cls, die):
        ...
        try:
            ...
            raise ParseException(
                "_get_type_die: DIE is not available in the typemap\n {}".format(
                    die.offset
                )
            )
