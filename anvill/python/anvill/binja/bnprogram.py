...
def _build_jump_table(bv, addr):
    """Get jump table for the given address"""
    jump_table = dict()
    for block in bv.get_basic_blocks_at(addr):
        ...
        while jump_addr < block.end:
            instr_info = bv.arch.get_instruction_info(bv.read(jump_addr, 16), jump_addr)
            # check if the instruction has branches
            if len(instr_info.branches) != 0:
                break

            jump_addr += instr_info.length

        if jump_addr >= block.end:
            continue

        for branch in instr_info.branches:
            if branch.type in [bn.BranchType.TrueBranch, bn.BranchType.FalseBranch]:
                branch_targets.add(branch.target)

        jump_table[jump_addr] = branch_targets
