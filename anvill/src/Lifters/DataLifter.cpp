...
DataLifter::GetOrDeclareData(const GlobalVarDecl &decl,
  ...
  std::stringstream ss;
  ss << remill::kAliasVariableNamePrefix << std::hex << decl.address << '_'
    ...
    if (base) {
      ...
      if (offset > 0) {
        ...
        LOG_IF(ERROR, offset < 0)
            << "Found negative offset for variable at " << std::hex
            << decl.address << std::dec
            << " cast and return to the correct data type.";
...
llvm::GlobalValue *DataLifter::LiftData(const GlobalVarDecl &decl,
  ...
  ss2 << remill::kGlobalVariableNamePrefix << std::hex << decl.address << '_'
      << TranslateType(*type, dl, true);
