...
void TypeSpecifierImpl::EncodeType(
  ...
  switch (type.getTypeID()) {
    ...
    case llvm::Type::FunctionTyID: {
      ...
      EncodeType(*func_ptr->getReturnType(), ss, alphanum);
      ss << (alphanum ? "_B" : ")");
      break;
    }
    ...
    case llvm::GetFixedVectorTypeId(): {
      ...
      ss << (alphanum ? "_G" : "<");
      EncodeType(*vec_ptr->getElementType(), ss, alphanum);
    ...
    case llvm::Type::ArrayTyID: {
      ...
      ss << (alphanum ? "_C" : "[");
      EncodeType(*array_ptr->getElementType(), ss, alphanum);
    ...
    case llvm::Type::PointerTyID: {
      ...
      // Get the type of the pointee.
        ...
        EncodeType(*elem_type, ss, alphanum);
...
const TypeDictionary &TypeSpecifier::Dictionary(void) const noexcept {
...
std::string TypeSpecifier::EncodeToString(
  ...
  if (type) {
    ...
    impl->EncodeType(*const_cast<llvm::Type *>(type), ss, alphanum);
  }
  return ss.str();
}
...
Result<llvm::Type *, TypeSpecificationError>
TypeSpecifier::DecodeFromString(const std::string_view spec) const {
  std::string decoded;
...
static llvm::Value *AdaptToType(llvm::IRBuilderBase &ir, llvm::Value *src,
                                llvm::Type *dest_type) {
