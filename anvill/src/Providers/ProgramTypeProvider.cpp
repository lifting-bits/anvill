...
struct ProgramTypeProvider::PrivateData final {
  PrivateData(const IProgram &program_, llvm::LLVMContext &context_)
      : program(static_cast<Program &>(program_)),
        context(context_) {}
...
ITypeProvider::Ptr ProgramTypeProvider::Create(const IProgram &program,
                                               llvm::Context &context) {
  return Ptr(new ProgramTypeProvider(program, context));
}
...
ProgramTypeProvider(llvm::LLVMContext &context_, const Program &program_)
    : d(new PrivateData(program, context)) {}
